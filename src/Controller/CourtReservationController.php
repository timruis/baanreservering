...
class CourtReservationController extends AbstractController
{
    ...
    public function RegisteredCourtReservation(EntityManagerInterface $em, Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $data = $form->getData();
            $startingDate = new \DateTime(date("Y/m/d H:i:s", strtotime( $data->getStartTime()->getTimestamp())));
            $endingDate = new \DateTime(date("Y/m/d H:i:s", strtotime(  $form->get('EndDate')->getData()->getTimestamp())));
